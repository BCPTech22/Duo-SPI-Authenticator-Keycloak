package com.keycloak.duospi.authenticator;
...
public class DuoAuthenticator implements org.keycloak.authentication.Authenticator{
    public static final DuoAuthenticator SINGLETON = new DuoAuthenticator();
    private final static Logger logger = Logger.getLogger(DuoAuthenticator.class);
    ...
    private String getRedirectUrlShim(AuthenticationFlowContext context, Boolean forceToken) {
        ...
        baseUrl = baseUrl + "realms/" + context.getRealm().getName() + "/bcp-duo-keycloak/callback";
    ...
    private Client initDuoClient(AuthenticationFlowContext context, String redirectUrl) throws DuoException {
        ...
        //default values
        String clientId = authConfig.getConfig().get(DuoAuthenticatorFactory.DUO_INTEGRATION_KEY);
        String secret = authConfig.getConfig().get(DuoAuthenticatorFactory.DUO_SECRET_KEY);
        String hostname = authConfig.getConfig().get(DuoAuthenticatorFactory.DUO_API_HOSTNAME);

        String overrides = authConfig.getConfig().get(DuoAuthenticatorFactory.DUO_CUSTOM_CLIENT_IDS);
    ...
    public void authenticate(AuthenticationFlowContext authenticationFlowContext) {
        ...
        if (authConfig.getConfig().getOrDefault(DuoAuthenticatorFactory.DUO_API_HOSTNAME, "none").equalsIgnoreCase("none")) {
            //authenticator not configured
            authenticationFlowContext.failure(AuthenticationFlowError.INTERNAL_ERROR);
        }
        if (authConfig.getConfig().getOrDefault(DuoAuthenticatorFactory.DUO_INTEGRATION_KEY, "none").equalsIgnoreCase("none")) {
            //authenticator not configured
            authenticationFlowContext.failure(AuthenticationFlowError.INTERNAL_ERROR);
        }
        if (authConfig.getConfig().getOrDefault(DuoAuthenticatorFactory.DUO_SECRET_KEY, "none").equalsIgnoreCase("none")) {
            //authenticator not configured
            authenticationFlowContext.failure(AuthenticationFlowError.INTERNAL_ERROR);
        }
    ...
    private void startDuoProcess(AuthenticationFlowContext authenticationFlowContext, String username) {
        ...
        try {
            ...
            if (authConfig.getConfig().getOrDefault(DuoAuthenticatorFactory.DUO_FAIL_SAFE, "true").equalsIgnoreCase("false")) {
